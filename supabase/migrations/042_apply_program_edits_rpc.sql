-- Transactional RPC for applying batched program edits.
-- All validations run first; any failure aborts the entire transaction with zero writes.
-- Used for editing published programs where changes must be atomic.
...
RETURNS jsonb
LANGUAGE plpgsql
SECURITY INVOKER  -- relies on existing RLS policies
AS $$
DECLARE
  v_remove record;
  v_log_count int;
...
BEGIN
  -- 0. Verify caller owns this program
  IF NOT EXISTS (
  ...
  -- Get client_id for date conflict checks
  SELECT client_id INTO v_program_client_id
  FROM programs WHERE id = p_program_id;

  -- ═══════════════════════════════════════════
  -- PHASE 1: VALIDATIONS (no writes yet)
  -- ═══════════════════════════════════════════

  -- 1a. Validate exercise removes — block if set_logs exist
  FOR v_remove IN SELECT * FROM jsonb_array_elements_text(p_exercise_removes)
  LOOP
    SELECT count(*) INTO v_log_count
    FROM set_logs
    WHERE template_exercise_id = v_remove.value::uuid;
  ...
  -- Check for duplicates within the pending changes
  SELECT d INTO v_dup
  ...
  LOOP
    ...
    WHERE client_id = v_program_client_id
      ...
      AND id != (v_date.value->>'scheduled_workout_id')::uuid
    LIMIT 1;
  ...
  -- 2a. Template updates (title, notes, scheduled_days)
  ...
  -- 2b. Exercise removes + position renormalization
  FOR v_remove IN SELECT * FROM jsonb_array_elements_text(p_exercise_removes)
  LOOP
    DELETE FROM workout_template_exercises WHERE id = v_remove.value::uuid;
  END LOOP;
  -- Renormalize positions for affected templates
  FOR v_remove IN
  ...
  LOOP
    -- positions already deleted, renormalize remaining
    NULL; -- handled below after all removes
  END LOOP;
  ...
  -- 2d. Exercise adds (appended to bottom)
