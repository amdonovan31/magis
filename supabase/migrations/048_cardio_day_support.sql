-- Add day type and cardio prescription columns to workout_templates.
-- Existing rows default to 'strength' — no data migration needed.
...
ALTER TABLE public.workout_templates
  ...
  ADD COLUMN IF NOT EXISTS cardio_distance_target numeric,
  ADD COLUMN IF NOT EXISTS cardio_distance_unit text,
  ADD COLUMN IF NOT EXISTS cardio_hr_zone int,
  ADD COLUMN IF NOT EXISTS cardio_notes text;

-- Cardio session logs — one row per completed cardio session.
-- Created now for schema completeness; client logging is a follow-up.
...
CREATE POLICY cardio_logs_client ON public.cardio_logs
  FOR ALL USING (
    session_id IN (SELECT id FROM public.workout_sessions WHERE client_id = auth.uid())
  );

CREATE POLICY cardio_logs_coach_read ON public.cardio_logs
  FOR SELECT USING (
    session_id IN (
      SELECT ws.id FROM public.workout_sessions ws
      JOIN public.coach_client_relationships ccr ON ws.client_id = ccr.client_id
      WHERE ccr.coach_id = auth.uid()
    )
  );

-- Update apply_program_edits() to support cardio columns in template updates.
